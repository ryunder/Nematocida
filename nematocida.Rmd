...
```
##Group by treatments
*All samples are of the same strain, temperature, RNA-seq lane
```{r}
...
1. First expression checks the number of genes that have zero counts in every condition.
...
Dimension 1 (x-axis in left plot) represents the largest contributor to the variance within the data. What this shows is that all ERTm1 and ERTm2 treated samples are quite similar to each other, while being quite different than untreated N2 samples. Differences in the replicates can be seen in the higher dimensions.
```{r}
lcpm <- cpm(x, log=T)
par(mfrow=c(1,2))
col.group <- treat
...
The variance of RNA-seq data isn ot independant of the log-cpm mean. This is visualized by the graph on the left, showing more variability in samples with lower counts. The voom function attempts to remmpve this heteroscedacity.
```{r}
par(mfrow=c(1,2))
